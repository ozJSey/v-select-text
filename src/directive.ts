 ...
 * `setSelectionRange`, a contenteditable host via either. Fires on a reactive
 * `false → true` transition, on every update with `trigger: 'always'`, or on
 * click with `trigger: 'click'`.
 *
 * @example
...
export const vSelectText: ObjectDirective<HTMLElement, SelectTextBinding> = {
  mounted(el, binding) {
    ...
    edgeSpentMap.set(el, cycle !== null)
  },
  ...
  updated(el, binding) {
    ...
    edgeSpentMap.set(el, resolved.enabled && (spent || cycle !== null))
  },
